...
class HOLODataset(BaseDataset):
    ...
    def __init__(self, data=None, task="detect", *args, **kwargs):
        """Initializes the HOLODataset with optional configurations for segments and keypoints."""
        self.use_segments = task != "detect"  # use segment for drivable and lane, except detect
        self.task = task
        self.data = data
        self.label_files = []
        super().__init__(*args, **kwargs)

    def cache_labels(self, path: Path = Path("./labels.cache"), task: str = "detect"):
        """
        Cache dataset labels, check images and read shapes.

        Args:
            path (Path): Path where to save the cache file. Default is Path('./labels.cache').
            task (str): Task name. Default is 'detect'.
    ...
    def multitask_img2label_paths(
        ...
        split = Path(sample_img).parent.stem
        image_ids = [Path(im_file).stem for im_file in img_files]

        return [
            str(prefix / split / task / f"{im_id}.txt")
...
def build_holo_dataset(
        cfg, img_path, batch, data, task: str = "detect",
        mode: str = "train", rect: bool = False, stride: int = 32
) -> HOLODataset:
    """Build HOLO Dataset."""
    return HOLODataset(
        data=data,
        task=task,  # task=cfg.task,
        img_path=img_path,
        imgsz=cfg.imgsz,
        batch_size=batch,
