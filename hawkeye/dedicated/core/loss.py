...
class MultitaskDetectionLoss:
    ...
    def __init__(self, multitask, task: str, device: torch.device):  # model must be de-paralleled
        ...
        self.hyp = h
        self.stride = task_head.stride  # model strides
        self.nc = task_head.nc  # number of classes
        self.no = task_head.nc + task_head.reg_max * 4
        self.reg_max = task_head.reg_max
        self.device = device

        self.use_dfl = task_head.reg_max > 1

        self.assigner = TaskAlignedAssigner(topk=10, num_classes=self.nc, alpha=0.5, beta=6.0)
        self.bbox_loss = BboxLoss(task_head.reg_max - 1, use_dfl=self.use_dfl).to(device)
        self.proj = torch.arange(task_head.reg_max, dtype=torch.float, device=device)
